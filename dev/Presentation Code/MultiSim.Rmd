...
### DEV CODE FOR VISUALIZATION - DELETE FOR PRESENTATION ###
# We should eventually have plots to visualize these data
coef_r <- coef(spls_model_r)
abs_coef_r <- abs(coef_r)
coef_cpp <- spls_model_cpp$betahat

nonzero_counts <- colSums(coef_r != 0)

View(spls_model_r$projection)
View(abs_coef_r)
```
